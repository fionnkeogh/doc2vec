...
paragraph2vec <- function(x,
  ...
  if(is.character(x)){
    ...
    idx         <- which(nwords >= 100000)
    if(length(idx) > 0){
      message(sprintf("Note: there are texts which are longer than 100000 words, for these we will take only the first 1000 words, example doc_id: %s", x$doc_id[sample(idx, size = 1)]))
      x$text[idx] <- sapply(strsplit(x$text[idx], split = "[ \t]"), FUN = function(x) paste(head(x, n = 100000), collapse = " "))    
    }
